...
void G4AntiNeutronChargeExchangeProcess::initialize(){
  ...
  method =  "BDT method";
  reader0->BookMVA( "BDT method", "/home/lliu/ustcfs/runarea/neutron/BesPip/myBesPip/AntiNeutronChargeExchange/share/dataset/weights/TMVAClassification_BDTG.weights.xml");
  fweight = new TFile("/home/lliu/ustcfs/runarea/neutron/BesPip/myBesPip/AntiNeutronChargeExchange/share/TMVA.root", "read");
  hB = (TH1D*)fweight->Get("dataset/Method_BDT/BDTG/MVA_BDTG_B_high");
  hS = (TH1D*)fweight->Get("dataset/Method_BDT/BDTG/MVA_BDTG_S_high");
  hB->Rebin(100);
  hS->Rebin(100);
...
G4VParticleChange* G4AntiNeutronChargeExchangeProcess::PostStepDoIt(const G4Track &aTrack,  const G4Step &aStep){
		...
		if(material->GetName() == "Oil"){  // Carb
				...
				while(jndex < 10000){
						...
						double classifier0 = reader0->EvaluateMVA( method );
						int idx = floor((classifier0 + 1.) / (2./100.));
						double reweight = hS->GetBinContent(idx);
						if(G4UniformRand() < reweight)
								break;
						jndex++;
				}
